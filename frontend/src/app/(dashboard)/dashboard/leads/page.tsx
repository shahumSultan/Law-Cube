...
function StatusSelect({
  ...
  const menu = open && createPortal(
    <div
      ...
      className="bg-white dark:bg-[#091a0f] border border-slate-200 dark:border-[#166534] rounded-xl shadow-xl py-1.5 min-w-[220px]"
    >
      {LEAD_STATUSES.map(s => (
        <button
          key={s}
          onClick={(e) => { e.stopPropagation(); onChange(s); setOpen(false); }}
          className={`w-full text-left px-3 py-2 flex items-center justify-between transition-colors hover:bg-slate-50 dark:hover:bg-[#0d2a18] ${
            s === value ? "bg-slate-50 dark:bg-[#0d2a18]/60" : ""
          }`}
        >
          <StatusPill status={s} />
    ...
    <tr className="border-b border-slate-50 dark:border-[#166534]/50">
      {[1, 2, 3, 4, 5].map(i => (
        <td key={i} className="px-4 sm:px-5 py-4">
          <div className="h-4 bg-slate-100 dark:bg-[#0d2a18] rounded animate-pulse" style={{ width: `${60 + i * 10}%` }} />
        </td>
      ))}
    </tr>
  );
...
export default function LeadsPage() {
  ...
  return (
    <div className="flex flex-col gap-5 max-w-[1400px]">
      ...
      {/* Filters */}
      <div className="bg-white dark:bg-[#091a0f] border border-slate-200 dark:border-[#166534] rounded-2xl p-3 sm:p-4">
        <div className="flex gap-2 sm:gap-3">
          <div className="flex items-center gap-2 bg-slate-50 dark:bg-[#0d2a18] border border-slate-200 dark:border-[#166534] rounded-xl px-3.5 py-2.5 flex-1 focus-within:border-[#15803d] transition-colors">
          ...
          <button
            ...
            className={`sm:hidden flex items-center gap-2 border rounded-xl px-3 py-2.5 text-sm font-medium transition-all font-sans-body ${
              hasActiveFilters ? "bg-[#15803d] border-[#15803d] text-white" : "bg-slate-50 dark:bg-[#0d2a18] border-slate-200 dark:border-[#166534] text-slate-600 dark:text-slate-400"
            }`}>
            <Filter className="w-3.5 h-3.5" />
          </button>
          <div className="hidden sm:flex items-center gap-2 sm:gap-3">
            <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }}
              className="bg-slate-50 dark:bg-[#0d2a18] border border-slate-200 dark:border-[#166534] rounded-xl px-3 py-2.5 text-slate-700 dark:text-slate-300 text-sm outline-none hover:border-slate-300 dark:hover:border-[#15803d] transition-colors cursor-pointer font-sans-body">
        ...
        {filtersOpen && (
          <div className="sm:hidden flex flex-col gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-[#166534]">
            <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }}
              className="w-full bg-slate-50 dark:bg-[#0d2a18] border border-slate-200 dark:border-[#166534] rounded-xl px-3 py-2.5 text-slate-700 dark:text-slate-300 text-sm outline-none font-sans-body">
              {STATUSES.map(s => <option key={s}>{s}</option>)}
            </select>
            <select value={sourceFilter} onChange={e => { setSourceFilter(e.target.value); setPage(1); }}
              className="w-full bg-slate-50 dark:bg-[#0d2a18] border border-slate-200 dark:border-[#166534] rounded-xl px-3 py-2.5 text-slate-700 dark:text-slate-300 text-sm outline-none font-sans-body">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 sm:px-6 py-4 border-t border-slate-100 dark:border-[#166534]">
          ...
          <div className="flex items-center gap-1.5 order-1 sm:order-2">
            <button
              ...
              disabled={page === 1}
              className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-[#0d2a18] border border-slate-200 dark:border-[#166534] flex items-center justify-center text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 hover:border-slate-300 transition-all disabled:opacity-40"
            >
              ...
              <button key={n} onClick={() => setPage(n)}
                className={`w-8 h-8 rounded-lg text-sm font-semibold font-sans-body transition-all ${
                  n === page
                    ...
                    : "bg-slate-50 dark:bg-[#0d2a18] border border-slate-200 dark:border-[#166534] text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"
                }`}>
                {n}
              </button>
            ))}
